...
## Amendment — 2026-07-15: evict the channel-list on re-invite (absent-window gap)

The 2026-07-14 amendment evicts on membership changes the bot *observes* — `channelCreate` / `channelDelete` / type-cross. It has a blind spot: while **no** edition's bot is in the guild (kicked/left), Discord fires no gateway event to the bot, so a channel created or deleted during that window is never invalidated. On re-invite the stale entry can survive up to its 5-min TTL, so a channel deleted while the bot was gone lingers as a phantom candidate in the dashboard for up to 5 min after the bot returns (and the same stale list would feed `Handover.evaluate`'s permission read on a premium re-invite).

**Decision:** `registerNewGuild` (the `guildCreate` re-invite path) calls `Discord.evictGuildChannels(guildId)` immediately after `activatePresence`, unconditionally. Re-invite is the exact point the absent window closes and a bot is confirmed present, so the next `cachedGet` re-fetches the live list via that present bot. Placed **before** `Handover.evaluate` (which reads the same `/guilds/:id/channels` entry) so the eval runs on fresh data, and before `pruneStaleChannels` (which is unaffected — it uses the bot-supplied GUILD_CREATE channel list, not the cache).

**Why re-invite, not kick.** Evicting on `guildDelete` would also work, but is redundant: the mutation happens *during* the absent window, and nothing can re-warm the entry with stale data while absent (a `cachedGet` refetch needs a member bot; a non-member's `/guilds/:id/channels` fetch 403s and `cachedGet` only stores 200s). Evicting when the window opens buys nothing over evicting when it closes — and kick-side eviction would cost a bot change + an HTTP round-trip, whereas re-invite eviction is backend-internal.

**Cost is negligible.** One `Map.delete` per re-invite (a rare lifecycle event), plus at most one extra live channel-list fetch if a handover eval or dashboard read follows within the old TTL window. The `/roles` and `/members/:botId` entries are untouched — permission freshness stays TTL-only, as the original decision requires.
