...
- `consume` is gated on the job's own `opts.priority`, never on a fresh budget read. An unconditional `DECR` would mint a negative key for every guild in the system, which is real memory inside the `allkeys-lru` budget and would evict keys that matter.
...
- Seeding uses plain `SET`, so a re-invite — and a premium bot joining on upgrade — re-arms the budget. Both are real join events, bounded at 10 publishes each.
- No Postgres migration; existing guilds are unaffected (no key → `NORMAL`).

## Related

The 5s embed delay over-fires badly for the same new-server audience (`RegExPatterns.url` matches `Node.js`, `README.md`, and any missing space after a period), which is the other half of what a boosted server experiences. Tracked separately — it touches `@ap/utils`, which both the bot hot path and filters depend on.
