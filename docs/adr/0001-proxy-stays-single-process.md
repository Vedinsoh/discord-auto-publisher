...
An earlier `refactor/v7` iteration experimented with splitting these concerns into two services: `proxy` (passthrough only) and `crosspost-worker` (queue + worker + classifier), with the worker calling the proxy over HTTP for the actual Discord crosspost call. This three-tier shape was speculative and was never load-tested in production. `v7` has since been consolidated to the single-process design captured here.
