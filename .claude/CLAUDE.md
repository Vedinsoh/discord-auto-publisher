...
- Crons: guild presence reconcile (`30 3 * * *` — one sweep per edition over `bot_presence` via that edition's proxy, then dangling-marker sweep + purge; also runs once at startup; manual trigger `POST /internal/reconcile/guilds`) and subscription reconcile (`0 4 * * *`).
