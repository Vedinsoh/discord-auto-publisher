...
**Guild presence + soft delete**: A `guild` row with `deletedAt IS NULL` means "bot is in this guild" (drives dashboard `botPresent` and the premium revocation backstop) — NOT "guild is migrated"; legacy guilds get rows too. Kick/leave soft-deletes (config + cache preserved for re-invite); re-invite (`guildCreate` → `registerNewGuild`) clears the soft delete, prunes config for channels deleted while the bot was away (live list from the GUILD_CREATE payload), and rebuilds the derived Redis state (entries first, marker last); the daily reconciliation cron inserts missed guilds as legacy, restores/soft-deletes by diffing Discord's guild list (rails: abort on pagination error, 1h join-race guard, `max(50, 10%)` deletion cap), and hard-purges rows soft-deleted >30 days ago (`Guilds.purge`, conditional on `deleted_at < cutoff` so a mid-sweep re-invite wins). `deletedAt` + reconciliation are permanent. See ADR 0005.
