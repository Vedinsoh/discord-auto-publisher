...
- **Plan catalogue** — one Paddle product ("Auto Publisher Premium"), two prices: $4.99/month, $47.88/year. No trial period; money-back requests are Paddle refunds. Price IDs are backend-only config (backend creates the transaction), not `NEXT_PUBLIC_*`.
