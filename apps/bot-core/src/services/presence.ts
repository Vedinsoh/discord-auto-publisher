...
const getGuildsCount = async () => {
  try {
    ...
    const res = await Data.API.Bot.getGuildsCount(clientId);
    const body = (await res.json()) as GuildsCountResponse;
...
const getTotalGuildsCount = async () => {
  try {
    const res = await Data.API.Bot.getTotalGuildsCount();
    const body = (await res.json()) as GuildsCountResponse;
...
const updateGuildsCount = async () => {
  try {
    ...
    await Data.API.Bot.updateGuildsCount(clientId, guildsCount);
  } catch (error) {
    client.logger.error(error);
  }
};
